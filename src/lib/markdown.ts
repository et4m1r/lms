...
export type ProductMdxFrontmatter = BaseMdxFrontmatter & {
  productId: number
  courseId: number
  cover: string
  categoryName: string
...
export async function getAllBlogsFrontmatter() {
  ...
  const uncheckedRes = await Promise.all(
    files.map(async (file) => {
      ...
      return {
        ...justGetFrontmatterFromMD<ProductMdxFrontmatter>(rawMdx),
        slug: file.split('.')[0],
      }
    }),
  )
  return uncheckedRes.filter((it) => !!it) as (ProductMdxFrontmatter & {
    slug: string
  })[]
}
...
export async function getCompiledBlogForSlug(slug: string) {
  ...
  try {
    ...
    return await parseMdx<ProductMdxFrontmatter>(rawMdx)
  } catch {
    return undefined
  }
}
...
export async function getBlogFrontmatter(slug: string) {
  ...
  try {
    const rawMdx = await fs.readFile(blogFile, 'utf-8')
    return justGetFrontmatterFromMD<ProductMdxFrontmatter>(rawMdx)
  } catch {
    return undefined
  }
}
